...
RUN apt-get --quiet --quiet --yes --no-install-recommends     --option "DPkg::Options::=--force-confold"     --option "DPkg::Options::=--force-confdef"     install apt-utils ca-certificates curl python3-pip liblinux-usermod-perl passwd
RUN apt-get --quiet --quiet --yes autoremove
RUN apt-get --quiet --quiet --yes clean
RUN rm -rf /var/lib/apt/lists/* 1>/dev/null

RUN useradd -m -s /bin/bash "ansible"     && echo ""ansible":"ansible"" | chpasswd     && /usr/sbin/usermod -aG sudo "ansible"
...
RUN PATH=/usr/local/bin:/usr/bin:/bin:/usr/local/games:/usr/games:/sbin:/usr/sbin:/home/ansible/.local/bin
#CMD ["sh", "./ansible"]
